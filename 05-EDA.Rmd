...
``` {r kruskalTest1, message=FALSE, warning=FALSE}
res <- kruskal.test(
...
``` {r dunnTest1, message=FALSE, warning=FALSE}
dunn_res <- dunnTest(
  `CO2 per capita` ~ Entity,
  data = income_data,
  method = "bonferroni"
...
``` {r kruskalTest2, message=FALSE, warning=FALSE}
res <- kruskal.test(
...
``` {r dunnTest2, message=FALSE, warning=FALSE}
dunn_res <- dunnTest(
  `CO2 emissions` ~ Entity,
  data = income_data,
  method = "bonferroni"
...
ggplot(plot_data, 
  ...
  theme(plot.title = element_text(hjust = 0.5))
```
Complementamos esta gráfica con una prueba de Chi-cuadrado de independencia con simulación de MonteCarlo, debido a la presencia de niveles con cantidades menores a 5, para evaluar si existe asociación entre la región y la distribución de grupos de ingresos mediante la siguiente prueba de Hipótesis:
...
``` {r tablaContingencia, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
tabla <- xtabs(
...
kable(
  tabla_df,
  caption = "Tabla de contingencia: Región vs Grupo de ingreso"
)
```

``` {r pruebaChiSquare, message=FALSE, warning=FALSE, fig.width=12, fig.height=5}
set.seed(42)
...
```
``` {r kruskalTest3, message=FALSE, warning=FALSE}
res <- kruskal.test(
...
``` {r dunnTest3, message=FALSE, warning=FALSE}
dunn_res <- dunnTest(
...
``` {r kruskalTest4, message=FALSE, warning=FALSE}
res <- kruskal.test(
...
``` {r dunnTest4, message=FALSE, warning=FALSE}
dunn_res <- dunnTest(
...
``` {r kruskalTest5, message=FALSE, warning=FALSE}
res <- kruskal.test(
...
``` {r dunnTest5, message=FALSE, warning=FALSE}
dunn_res <- dunnTest(
  `Temperature change` ~ factor(`Region`),
  data = projected_climate_change_country_data,
  method = "bonferroni"
...
``` {r kruskalTest6, message=FALSE, warning=FALSE}
res <- kruskal.test(
...
``` {r dunnTest6, message=FALSE, warning=FALSE}
dunn_res <- dunnTest(
  `Temperature change` ~ factor(`Income group`),
  data = projected_climate_change_country_data,
  method = "bonferroni"
...
``` {r kruskalTest7, message=FALSE, warning=FALSE}
res <- kruskal.test(
...
``` {r dunnTest7, message=FALSE, warning=FALSE}
dunn_res <- dunnTest(
  `Precipitation change` ~ factor(`Region`),
  data = projected_climate_change_country_data,
  method = "bonferroni"
...
``` {r kruskalTest8, message=FALSE, warning=FALSE}
res <- kruskal.test(
